...
{
	M_ASSERT((SDL_Init(SDL_INIT_EVERYTHING) >= 0), "Init Error");
	...
	m_renderer = SDL_CreateRenderer(GetWindow(), -1, 0);
...
void Renderer::EmumerateDisplayModes()
{
...
void Renderer::RenderTexture(Texture* _texture, Point _point)
{
	...
	m_destRect.w = _texture->GetImageInfo()->Width; //not minus x
